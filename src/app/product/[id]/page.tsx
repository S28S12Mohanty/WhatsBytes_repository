    ...
    <div>
      ...
      <main className="container px-4 mt-8 grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="space-y-3">
          <div className="relative aspect-[4/3] rounded-xl overflow-hidden border border-neutral-800">
            <Image
          ...
          <div className="grid grid-cols-4 gap-3">
            {product.images.map((src, idx) => (
              <div key={idx} className="relative aspect-square rounded-lg overflow-hidden border border-neutral-800">
                <Image src={src} alt={`${product.title} ${idx + 1}`} fill className="object-cover" />
              </div>
            ))}
          </div>
        </div>
